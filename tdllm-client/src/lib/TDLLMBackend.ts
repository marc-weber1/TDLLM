...
async function generate_cli(
  test_code: string,
  prompt: string
): Promise<{ code: string; test_info: ProcessResult; iterations: number }> {
  return await call_api("generate_and_test", { tests: test_code, prompt });
}

async function generate_api(
  test_code: string,
  prompt: string
): Promise<{
  code: string;
  server_info: ProcessResult;
  test_info: ProcessResult;
  iterations: number;
}> {
